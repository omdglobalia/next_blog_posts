...
const Dashboard = () => {
    ...
    try {
      await fetch('/api/posts', {
        ...
        body: JSON.stringify({
          ...
          userImg: session.status === "authenticated" ? session.data.user.image : ""
        })
      })
      mutate()
      e.target.reset()
    } catch (error) {
      console.log("file: page.jsx:60 ~ handleSubmit ~ error:", error)
    }
  }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.log("file: page.jsx:78 ~ handleDelete ~ error:", error)
